...
KATE:registerModule(KateCoreAeroFlightModule@).

include ("kate/modules/kate_module").
include ("kate/core_modules/flight/aero_autopilot_task").
...
global function KateCoreAeroFlightModule {
    local this is KateModule("KateCoreAeroFlightModule", "FLTAERO").

    this:taskCreators:add("ATPLT", KateAeroAutopilotTask@).
    ...
    // Overrides
    this:override("updateState", KateCoreAeroFlightModule_updateState@).
    this:override("renderModuleUi", KateCoreAeroFlightModule_renderModuleUi@).
    this:override("handleModuleInput", KateCoreAeroFlightModule_handleModuleInput@).
    this:override("handleMessage", KateCoreAeroFlightModule_handleMessage@).

    this:def("changeAutopilotParameter", KateCoreAeroFlightModule_changeAutopilotParameter@).
    this:def("setAutopilotParameterFromMessage", KateCoreAeroFlightModule_setAutopilotParameterFromMessage@).
    this:def("changeFlightPlanWaypoint", KateCoreAeroFlightModule_changeFlightPlanWaypoint@).

    return this.
}

local function KateCoreAeroFlightModule_updateState {
    parameter   this.
...
local function KateCoreAeroFlightModule_renderModuleUi {
    parameter   this.
    ...
    if result:empty {
        result:add("No AERO func available: " + ship:status).
    }
...
local function KateCoreAeroFlightModule_handleModuleInput {
    parameter   this,
                runtime,
                inputCharacter.
...
local function KateCoreAeroFlightModule_handleMessage {
    parameter   this,
                runtime,
                message.
...
local function KateCoreAeroFlightModule_changeAutopilotParameter {
    parameter this,
              name,
              delta,
              modulo is 0.
...
local function KateCoreAeroFlightModule_setAutopilotParameterFromMessage {
    parameter this,
              message,
              name.
...
local function KateCoreAeroFlightModule_changeFlightPlanWaypoint {
    parameter this,
              mode,
              value.
